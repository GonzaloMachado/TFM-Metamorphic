...
def main():
    query_string = "select col1, col2 || col3 , col4 + col5, col6  from persona AS tabla1, departamento where edad between 25 and 35"
    print(query_string)
    parsed_tree = psqlparse.parse(query_string)
    allData = list()
    for statement in parsed_tree:
        allData.append(get_info(statement))
    # equivalent = select_match_case(allData[0])
    x = PrettyPrinter()
    # equivalent2 = select_match_case_2(parsed_tree[0], x)
...
def select_match_case_2(statement, printer):
    query = 'SELECT '
    first = True
    not_case_found = True
    if statement.target_list is not None:
        for field in statement.target_list:
            if first:
                first = False
            else:
                query += ', '
            if not_case_found:
                # if field['type'] == 'AStar':
                #     query += '*'
                # if field['type'] == 'ColumnRef':
                #     query += f"{field['alias']}.{field['name']} " if field['alias'] is not None else field['name']
                if isinstance(field.val, ColumnRef):
                    print('Columna simple, Do Print')
                    # print(print_node(field))
                if isinstance(field.val, AExpr):
                    if field.val.name[0].val == '||':
                        print("Caso #2 Found")
                    else:
                        print("Caso #1 Found")
                    # query += caso_uno(field)
                    # not_case_found = False
                    # print(query)
                # elif field['type'] == 'FuncCall':
                #     print("Caso #2")
                #     not_case_found = False
                #     query += caso_dos(field)
            # else:
            #     if field['type'] == 'FuncCall':
            #         query += print_func_call(field)
            #     if field['type'] == 'AExpr':
            #         query += print_a_expr(field)
...
def caso_uno(expression):
    if isinstance(expression['lexpr'], dict):
        equivalent_statement = caso_uno(expression['lexpr']) + f" {expression['arithoper']} COALESCE({expression['rexpr']},0)"
    else:
